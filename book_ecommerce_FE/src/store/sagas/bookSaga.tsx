...
function* fetchBooksSaga(
  ...
  try {
    ...
    const response: AxiosResponse<ApiResponse<Book[]>> = yield call(
      api.get,
      "/books",
      {
        params: {
...
function* fetchBookByIdSaga(action: PayloadAction<number>): SagaIterator {
  try {
    ...
    yield put(fetchBookByIdSuccess(response.data.data as Book));
  } catch (error: any) {
    yield put(
      fetchBookByIdFailure(
...
function* fetchBooksByGenreSaga(
  action: PayloadAction<{ genreId: number }>
): SagaIterator {
  try {
    ...
    yield put(
      fetchBooksByGenreFailure(
        error.response?.data?.message || "Lỗi khi tải sách theo thể loại"
      )
    );
  }
}
...
function* createBookSaga(action: PayloadAction<BookFormData>): SagaIterator {
  try {
    ...
    formData.append(
      ...
      bookData.discount_percentage.toString()
    );
    ...
    if (bookData.sample_pages) {
      const files = Array.isArray(bookData.sample_pages)
        ? bookData.sample_pages
        : Array.from(bookData.sample_pages);
      files.forEach((file) => {
...
function* updateBookSaga(
  action: PayloadAction<{ id: number; book: BookFormData }>
): SagaIterator {
  try {
    ...
    formData.append(
      "book[discount_percentage]",
      book.discount_percentage.toString()
    );
    ...
    if (book.sample_pages) {
      const files = Array.isArray(book.sample_pages)
        ? book.sample_pages
        : Array.from(book.sample_pages);
      files.forEach((file) => {
        formData.append("book[sample_pages][]", file);
      });
    }
...
function* deleteBookSaga(
  ...
  try {
    ...
    yield put(
      deleteBookFailure(error.response?.data?.message || "Lỗi khi xóa sách")
    );
  }
}
